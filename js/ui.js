...
const UI = {
  ...
  formatPrice(price, modality) {
    if (modality === 'Precio total del lote') {
      return `Bs ${price.toLocaleString('es-BO')} (Lote total)`;
    }
      ...
      <article class="lot-card" data-lot-id="${lot.id}">
        <div class="lot-card-media" onclick="App.openLotDetail('${lot.id}')">
          ...
          <button class="btn-fav ${isFavorite ? 'active' : ''}" 
            ...
            <svg viewBox="0 0 24 24" width="20" height="20" fill="${isFavorite ? '#e06d3b' : 'none'}" stroke="${isFavorite ? '#e06d3b' : 'currentColor'}" stroke-width="2">
  ...
  renderSellerLotCard(lot) {
    ...
    return `
      <div class="seller-lot-card" data-lot-id="${lot.id}">
        ...
        <div class="seller-lot-details">
          <div class="seller-lot-main">
            ...
            <p class="seller-lot-price">${this.formatPrice(lot.price, lot.priceModality)}</p>
            <p class="seller-lot-stock">${lot.quantity} ${lot.unit} disponibles • ${lot.location}</p>
            <p class="seller-lot-meta">Publicado: ${this.formatDate(lot.publishedAt)} • ${lot.viewsCount || 0} vistas • ${lot.favoritesCount || 0} guardados</p>
            ${lot.status === 'sold' && lot.declaredSoldQuantity ? `
              <p class="declared-sold-note">✓ Venta declarada: ${lot.declaredSoldQuantity} ${lot.unit}</p>
            ` : ''}
          </div>
          ...
          <div class="seller-lot-actions">
            ${lot.status === 'active' ? `
              ...
              <button class="btn btn-sm btn-outline" onclick="App.openFeaturedModal('${lot.id}')">
                ★ Destacar
              </button>
      ...
      <div class="simulated-map-container">
        <div class="map-header">
          <div class="map-title-wrap">
            ...
            <span class="map-sub">Mapa demostrativo de lotes activos</span>
          </div>
          <span class="map-count">${activeLots.length} ubicaciones activas</span>
        </div>
        ...
        <div class="map-canvas">
          <svg viewBox="0 0 800 500" class="tarija-svg-map">
            ...
            <path d="M100 50 C250 120, 380 220, 520 280 C650 340, 720 480, 750 500" 
                  fill="none" stroke="#7ac1eb" stroke-width="24" stroke-linecap="round" opacity="0.6"/>
            <path d="M100 50 C250 120, 380 220, 520 280 C650 340, 720 480, 750 500" 
                  fill="none" stroke="#5da9dc" stroke-width="12" stroke-linecap="round" opacity="0.9"/>
            <text x="360" y="195" fill="#3a82b3" font-size="12" font-weight="600" font-family="system-ui" transform="rotate(30 360 195)">Río Guadalquivir</text>

            <!-- Zone Areas -->
            <!-- Mercado Campesino Zone -->
            <rect x="340" y="130" width="160" height="110" rx="16" fill="#1b7a43" fill-opacity="0.12" stroke="#1b7a43" stroke-width="2" stroke-dasharray="4"/>
            <text x="420" y="160" fill="#1b7a43" font-size="13" font-weight="700" text-anchor="middle" font-family="system-ui">Zona Mercado Campesino</text>
            <text x="420" y="178" fill="#558b2f" font-size="10" text-anchor="middle" font-family="system-ui">Principal centro de acopio</text>

            <!-- Centro & Mercado Central -->
            <rect x="280" y="260" width="140" height="90" rx="16" fill="#e06d3b" fill-opacity="0.12" stroke="#e06d3b" stroke-width="2" stroke-dasharray="4"/>
            <text x="350" y="295" fill="#c05621" font-size="12" font-weight="700" text-anchor="middle" font-family="system-ui">Centro / Mercado Central</text>
            ...
            <rect x="480" y="310" width="150" height="90" rx="16" fill="#48bb78" fill-opacity="0.12" stroke="#38a169" stroke-width="1.5" stroke-dasharray="4"/>
            ...
            <!-- Main Tarija Avenues -->
            <path d="M120 180 L680 180" stroke="#cbd5e1" stroke-width="4" stroke-linecap="round"/>
            <text x="600" y="172" fill="#94a3b8" font-size="9" font-family="system-ui">Av. Circunvalación</text>
            <path d="M420 50 L420 460" stroke="#cbd5e1" stroke-width="4" stroke-linecap="round"/>
            <text x="428" y="90" fill="#94a3b8" font-size="9" font-family="system-ui">Av. Froilán Tejerina</text>
          ...
          <div class="map-pins-layer">
            ${activeLots.map((lot, idx) => {
              const coords = lot.coordinates || { x: 30 + (idx * 5) % 50, y: 30 + (idx * 7) % 50 };
              return `
                <button class="map-pin-btn ${lot.quickExit ? 'pin-urgent' : ''}" 
                        style="left: ${coords.x}%; top: ${coords.y}%;"
                        title="${lot.product} - ${lot.priceModality} ${lot.price}"
                        onclick="App.openLotDetail('${lot.id}')">
                  <span class="pin-icon">${lot.quickExit ? '⚡' : '🧺'}</span>
        ...
        <p class="map-note">Simulación visual orientativa para el piloto de Tarija. Haz clic en cualquier marcador para abrir el detalle del lote.</p>
      </div>
    `;
  },

  // --- Image Compressor Helper ---
  compressImage(file, maxWidth = 900, maxHeight = 700, quality = 0.75) {
